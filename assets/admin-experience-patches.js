 ...
 * Splits Admin into Manage People / Quarter Dashboard
 * and adds admin daily-digest notification settings.
 */
(function () {
  "use strict";
  ...
  function showAdminSection(mode) {
    ...
    if (parts.title) {
      parts.title.textContent = peopleMode
        ? "Manage People"
        : "Quarter Dashboard";
    }

    if (parts.adminTop) {
      parts.adminTop.style.display = peopleMode ? "" : "none";
    }
    ...
    if (parts.peopleList) {
      parts.peopleList.style.display = peopleMode ? "" : "none";
    }

    if (parts.quarterControls) {
      parts.quarterControls.style.display = peopleMode ? "none" : "";
    }

    if (parts.quarterDashboard) {
      parts.quarterDashboard.style.display = peopleMode ? "none" : "";
    }
  ...
  function installAdminTopButtons() {
    const oldButton = document.getElementById("adminButton");
    if (!oldButton || !isAdmin()) return;

    oldButton.style.display = "none";

    if (!document.getElementById("managePeopleTopButton")) {
      const peopleButton = document.createElement("button");
      peopleButton.id = "managePeopleTopButton";
      peopleButton.className = oldButton.className || "admin-button";
    ...
    if (!document.getElementById("quarterDashboardTopButton")) {
      const dashboardButton = document.createElement("button");
      dashboardButton.id = "quarterDashboardTopButton";
      dashboardButton.className = oldButton.className || "admin-button";
  ...
  async function loadAdminDigestPreferences() {
    ...
    if (error) {
      console.warn("Unable to load admin daily digest preferences:", error);
      return;
    }
  ...
  async function saveAdminDigestPreferences() {
    ...
    try {
      ...
      if (typeof showAccountMessage === "function") {
        showAccountMessage(
          enabled.checked
            ? `Daily schedule summaries are on. The digest will be sent to ${destination}.`
            : "Daily schedule summary emails are off."
        );
      }
    } catch (error) {
      console.error(error);

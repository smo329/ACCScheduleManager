...
function renderIntoCenter(){
  ...
  section.querySelectorAll('.mark-reset-read').forEach(b=>b.onclick=async()=>{const id=b.closest('[data-password-reset-id]')?.dataset.passwordResetId;if(!id)return;await supabaseClient.from('notification_log').update({read_at:new Date().toISOString(),status:'read'}).eq('id',id).eq('user_id',currentUser.id);await refresh()});
}
